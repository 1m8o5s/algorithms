...
if (graph_marks.size()>=remove){
subgraphs_marks.resize(graph_marks.size()-remove);
for (int i = 0; i < graph_marks.size()-remove; i++){
for (int j = 0; j < graph_marks.size()-remove-i-1; j++){
subgraphs_marks[i].push_back(graph_marks[j]);
}
for (int j = graph_marks.size() -remove - i; j < graph_marks.size(); j++){
subgraphs_marks[i].push_back(graph_marks[j]);
}
}
for (int i = 0; i < subgraphs_marks.size(); i++){
sum += count_(subgraphs_marks[i], graph, i+remove);
}
return sum;
}else{
  return sum;
